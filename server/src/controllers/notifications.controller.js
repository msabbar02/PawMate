/**
 * Controlador de notificaciones por email para reservas.
 *
 * Envía emails al dueño cuando el cuidador acepta o rechaza una reserva,
 * al cuidador cuando recibe una nueva solicitud, y al dueño cuando el
 * servicio termina para que valore la experiencia.
 */
 ...
 *
 * Notifica por email al dueño (aceptación/rechazo) y al cuidador
 * (nueva solicitud pendiente) según el estado actual de la reserva.
 *
 * @param {import('express').Request}  req Petición con `{ reservationId }`.
 * @param {import('express').Response} res Respuesta con flags `{ owner, caregiver }`.
 */
const sendReservationStatusEmail = async (req, res) => {
    try {
        ...
        // Verifica que el usuario que hace la petición es participante de la reserva.
        if (req.user.uid !== data.ownerId && req.user.uid !== data.caregiverId) {
            return sendError(res, 'Not authorized for this reservation', 403);
        }

        // Busca en paralelo el email del dueño y del cuidador en la tabla users.
        ...
        // Email al dueño cuando la reserva ha sido aceptada o rechazada.
  ...
  <p style="color:#64748b;font-size:13px;margin:0;">¿Tienes alguna pregunta? Escríbenos a <a href="mailto:soporte@apppawmate.com" style="color:#16A34A;">soporte@apppawmate.com</a></p>`,
                    });
  ...
  <p style="color:#64748b;font-size:13px;margin:0;">¿Necesitas ayuda? <a href="mailto:soporte@apppawmate.com" style="color:#F5A623;">soporte@apppawmate.com</a></p>`,
        ...
        // Email al cuidador con la nueva solicitud de reserva pendiente.
 ...
 *
 * Envía al dueño un email pidiéndole que valore al cuidador tras finalizar
 * el servicio. Si no hay email válido del dueño, responde sin enviar.
 *
 * @param {import('express').Request}  req Petición con `{ reservationId }`.
 * @param {import('express').Response} res Respuesta con `{ sent }`.
 */
